...
bool collision_LineCollision( Vector2* l0p0, Vector2* l0p1, Vector2* l1p0, Vector2* l1p1, Vector2* outCollPos, float* outT )
{
	// based on the line segment collision above, only we don't bother testing if t is in range
	assert( l0p0 != NULL );
	assert( l0p1 != NULL );
	assert( l1p0 != NULL );
	assert( l1p1 != NULL );
	...
	//  2. r x s == 0 and (q-p) x r != 0
	//     Lines are parallel and non-intersecting
	//  3. r x s != 0 then the lines meet at p+tr=q+us
